...
\begin{document}
	...
	\par \begin{enumerate}[topsep=1px]
		...
		\item Imprimis autem hic notatu dignum occurrit, nostram curva triangularem esse rectisicabilem, quippe cuius arcu aequalis est radio osculi $MU$ curvae orbiformis, unde est nata. Videmus autem esse
		\begin{align*}
		...
		\]
		\par qui ergo longitudinem nostrae curvae triangularis exprimit; id quod etiam patet ex proprietate supra observata, quod sit $\D t = p\, \D u$, unde sit elementum curvae
		
		\[
			\sqrt{\D t^2+\D u^2} = \D u\sqrt{1+pp} = -\frac{3p\, \D S}{\D p}\sqrt{1+pp}-\frac{\D^3S}{\D p^2}(1+pp)^\frac{3}{2}
		\]
		cuius integral manifesto est
		\[
		...
		\item Quoniam hic tantum curvas triangulares investigare instituimus, param solliciti, utrum sint rectificabiles nec ne, dummodo suerint algebraicae: hac conditone omissa simpliciores formulas pro coordinatis $t$ et $u$ exhibere, atque adeo, sine ullo respectu ad curvas orbiformes habito, directe ex isa indole harum curvarum elicere poterimus. Cum enim esse debeat $\operatorname{d}t=p \operatorname{d}u$, erit $t=\int p\operatorname{d}u = pu-\int u \operatorname{d}p$. Iam statuamus $\int u \operatorname{d}p = \Pi$, ita ut sit $u=\frac{\operatorname{d} \Pi}{\operatorname{d}p}$, unde sit $t=\frac{p\operatorname{d} \Pi}{\operatorname{d}p}-\Pi$; ubi pro $\Pi$ eusmodi functiones ipsius $p$ accipi debent, quae nullo casu siant infinitae, quincunque valores literae $t$ tribuantur, cuiusmodi functiones iam supra descipsimus; tum vero etiam hae functiones $\Pi$ nulla signa radicalia, quae ambiguitatem involuant, involuere debent. Imprimis autem necesse est, ut ambae coordinatae $t$ et $u$ tribus casibus fiant maximae vel minimae, id quod eueniet, si, ob $u=\frac{\operatorname{d} \Pi}{\operatorname{d}p}$, haec aequatio: $\frac{\operatorname{dd}\Pi}{\operatorname{d}p^2} = 0$, tres habeat radices reales, neque vero plures.

		\switchenum
		\item a
		\switchcolumn*
		
		\item Sumamus expmpli gratia $\Pi = \frac{a+bp}{1+fp+gpp}$, quae nullo casu sit infinita, si modo suerit $ff< 4g$, tum autem erit
		\[
			\frac{\operatorname{d}\Pi}{\operatorname{d}p} = \frac{b-af-2agp-bgpp}{(1+fp+gpp)^2} = u
		\]
		hincque
		\[
			t = \frac{-a-2afp-3agp^2-2bgp^3}{(1+fp+gpp)^2}.
		\]
		\par Ut iam ternas cuspides desiniamus, consideremus aequationem $\frac{\operatorname{d}u}{\operatorname{d}p} = 0$, quod quo facilius fieri possit ponamus
		\[
		...
		\]
		cuius tres radices nobis ternas cuspides mostrabunt.
		...
		\]
		\[
			\alpha\beta+\alpha \gamma+\beta \gamma = -\alpha^2 \quad \text{et} \quad \alpha+\beta+ \gamma = 0,
		\]
		\par eritque postrema aequatio $3f = 3\alpha \alpha f g$, fiue $f=\alpha \alpha fg$; unde sequitur vel $f= 0$, vel $g= \frac{1}{\alpha \alpha}$. Consideremus primo casum $f=0$, fietque $A = -\frac{0}{0}$, unde littera $A$ non determinatur, vel potius sit $A = 0$, porroque $B=0$, unde colligeitur $b=0$, siue etiam $b$ non determinatur; tum vero erit $a=0$. Quia autem aequationem postremam per $f$ multiplicauimus, hic valor $f=0$ lubricus est habendus. Sumamus igitur alterum valorem $g= \frac{1}{\alpha \alpha}$, et quia debet esse $ff<4g$, sequitur esse debere $f< \frac{2}{\alpha}$; hinc vero siet $A=0$ et $B=0$, ideoque $b-af = 0$, et $et-2ag = 0$, unde sit $a = 0$.
		...
		\[
			\Pi = \frac{bp}{\alpha \alpha + pp},\quad \text{unde sit} \quad \frac{\operatorname{d}\Pi}{\operatorname{d}p} =u = \frac{b(\alpha\alpha- pp)}{(\alpha\alpha+pp)^2}\quad \text{et}
		\]
		\[
			t = \frac{2bp^3}{(\alpha \alpha+pp)^2}.
		\]

		\par Quod si iam pro cuspidibus faciamus $\frac{\operatorname{d}u}{\operatorname{d}p}=0$, nascitur haec aequatio: $p^3-3\alpha \alpha p = 0$, cuius ternae radices sunt
		\[
			\text{I}\degree.\; p=0;\quad \text{II}\degree.\; p=+\alpha\sqrt{3};\quad  \text{III}\degree.\; p=-\alpha \sqrt{3}
		\]
		pro quarum prima habebimus $t=0$ et $u=\frac{b}{\alpha \alpha}$;
		pro secunda:
		\[
		...
		\]
		unde curva habebit formeam in figura 8 delineatam, ubi est
		\[
			FB = \frac{b}{\alpha\alpha}, FG = FH = \frac{3b\sqrt{3}}{8\alpha}ac
		\]
		...
		\]
		\par ita ut haec figura triangularis traingulum isosceles exhibeat.
